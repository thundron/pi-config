---
name: claude-code
description: "Delegate coding tasks to Anthropic's Claude Code CLI from pi. Use when the user wants to hand off a coding task — fix a bug, refactor, write tests, review a PR, or run a multi-turn agentic session — to claude code (which runs under the user's Claude Pro/Max subscription via OAuth). Covers print mode (-p) for one-shots, tmux orchestration for interactive sessions, PTY dialog handling, parallel instances, hooks, MCP, custom subagents, and slash commands."
license: MIT
---

# Claude Code — pi Orchestration Guide

Delegate coding tasks to [Claude Code](https://code.claude.com/docs/en/cli-reference) (Anthropic's autonomous coding agent CLI) from pi via its `bash` tool. Claude Code v2.x can read files, write code, run shell commands, spawn subagents, and manage git workflows autonomously.
...
pi interacts with Claude Code in two fundamentally different ways. Choose based on the task.
...
1. **Interactive mode REQUIRES tmux** — Claude Code is a full TUI app. tmux gives you `capture-pane` for monitoring and `send-keys` for input, which is essential for orchestration from pi's `bash` tool (which is not a PTY).
...
3. **Always `cd` into the project dir first** — `cd /path/to/project && claude -p '...'`. Keep Claude focused on the right tree.
